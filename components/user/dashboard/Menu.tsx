...
export type MenuKeyType = 'dashboard' | 'projects' | 'myTasks' | 'calendar' | 'timeManage' | 'reports' | 'settings'
type MenuValueType = {
  label: string
  icon: string
}
...
export const Menu: React.FC<Props> = ({menu, handleOnClick}: Props) => {
  ...
  for (let key in MenuMap) {
    memuElements.push(
      <MenuItem menuKey={key} isActive={key === menu} handleOnClick={handleOnClick} />
    )
  }
